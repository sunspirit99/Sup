...
import (
	"encoding/json"
	"fmt"
	cfg "hyperledger/SupplyChain/Config"
	m "hyperledger/SupplyChain/Model"
	"hyperledger/SupplyChain/fabric"
	"log"
	"strconv"
	"time"
...
func (api AccountAPI) AddTransaction(tran m.Transactions) (m.Transactions, error) {
	...
	if err != nil {
		log.Fatalln("Failed to create transaction in fabric network !", err)
	}
...
func (api AccountAPI) GetActionDetail_Fabric(id string) (m.Diaries, error) {

	var diary m.Diary_Fabric
	...
	response, err := contract.EvaluateTransaction("GetActionDetail", id)
	if err != nil {
		fmt.Println("Failed to process in fabric !")
	}
	err = json.Unmarshal(response, &diary)

	return FabricToModel(diary), err
}

func (api AccountAPI) GetDiaryByProduct_Fabric(product string) ([]m.Diaries, error) {
	var diaryFabric []m.Diary_Fabric
	...
	response, err := contract.EvaluateTransaction("GetDiaryByProduct", product)
	if err != nil {
		fmt.Println("Failed to process in fabric !")
	}
	err = json.Unmarshal(response, &diaryFabric)

	var result []m.Diaries
	for _, d := range diaryFabric {
		temp := FabricToModel(d)
...
func FabricToModel(diaryFabric m.Diary_Fabric) m.Diaries {

	t, err := time.Parse(time.RFC3339, diaryFabric.Created_time)
	if err != nil {
		fmt.Println("Error when parsing string to time :", err)
	}

	return m.Diaries{
		Id:               diaryFabric.Id,
		Title:            diaryFabric.Title,
		Content:          diaryFabric.Content,
		Created_by:       diaryFabric.Created_by,
		Created_time:     &t,
		Related_area:     diaryFabric.Related_area,
		Related_batch:    diaryFabric.Related_batch,
		Related_products: diaryFabric.Related_products,
	}
}
