...
import (
	...
	controller "hyperledger/SupplyChain/Controller"
	model "hyperledger/SupplyChain/Model"
	"io/ioutil"
	"log"
	"net/http"
...
func GetDiaryByProduct_Fabric(w http.ResponseWriter, r *http.Request) {
	var diary []model.Diaries

	vars := mux.Vars(r)
	productID := vars["productID"] // Get from URL path

	diary, err := controller.NewAccountAPI().GetDiaryByProduct_Fabric(productID)
	if err != nil {
		log.Println(err)
	}
	Response(w, diary)
...
func GetActionDetail_Fabric(w http.ResponseWriter, r *http.Request) {
	var diary model.Diaries

	vars := mux.Vars(r)
	id := vars["id"] // Get from URL path

	diary, err := controller.NewAccountAPI().GetActionDetail_Fabric(id)
	if err != nil {
		log.Println(err)
	}
	Response(w, diary)
